...
public class PriceCommand {
            ...
            dispatcher.register(
                ClientCommandManager.literal("gachi")
                    .then(ClientCommandManager.literal("vote")
                        ...
                        .executes(ctx -> { VoteCommand.sendHelp(ctx.getSource()); return 1; })
                    )
            );
        });
    }
    ...
    private static int executeSearch(CommandContext<FabricClientCommandSource> ctx) {
        String raw = StringArgumentType.getString(ctx, "item").trim().replace("_", " ");
    ...
    private static void sendHelp(FabricClientCommandSource source) {
        ...
        source.sendFeedback(Text.literal("━━━━━━━━━━━━━━━━━━━━━━━━━━").formatted(Formatting.DARK_GREEN));
        source.sendFeedback(Text.literal(" /gachi price ").formatted(Formatting.YELLOW).append(Text.literal("<아이템이름>").formatted(Formatting.WHITE)));
        source.sendFeedback(Text.literal(" 이름 일부만 입력해도 검색됩니다. (띄어쓰기 → _)").formatted(Formatting.GRAY));
        source.sendFeedback(Text.literal("  /gachi price 다이아").formatted(Formatting.DARK_GRAY));
        source.sendFeedback(Text.literal("  /gachi price 네더라이트_주괴").formatted(Formatting.DARK_GRAY));
        source.sendFeedback(Text.literal(" /gachi vote ").formatted(Formatting.YELLOW).append(Text.literal("<아이템이름> <가격>").formatted(Formatting.WHITE)));
        source.sendFeedback(Text.literal(" 아이템 적정 가격을 투표합니다. (띄어쓰기 → _)").formatted(Formatting.GRAY));
